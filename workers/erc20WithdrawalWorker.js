...
var processWithdrawal = function (trans, request, withdrawalKeys) {
    database('localhost', 'main').then(function (db) {
            ...
            .then(function (row) {
                ...
                row.save(function (err) {
                    if (err) {
                        ...
                        let erc20AccreditPayment;
                        erc20AccreditPayment = InfuraIO.eth_sendRawTransaction(row.id, row.amount, row.token_type, withdrawalKeys.erc20, config.erc20WithdrawalWallet, row.withdrawal_address, 18, trans);
                        erc20AccreditPayment
                    ...
                    .then(function (items) {
                        ...
                        if (items && items.length) {
                            for (var i = 0; i < items.length; i++) {
                                promises.push(InfuraIO.eth_getTransactionByHash(items[i].extra.hash, items[i].id));
                            }
                        }
                        Promise.all(promises)
                            .then(function (data) {
                                ...
                                for (var j = 0; j < data.length; j++) {
                                    if (data[j] && data[j].result && data[j].result.blockNumber) {
                                        (function (dataj) {
                                            db.models.gateway_erc20_withdrawal.getAsync(dataj.id)
                                                .then(function (row) {
                                                    console.log(dataj);
                                                    console.log(row);
                                                    row.withdrawal_success = true;
                                                    row.status = 1;
                                                    row.extra = dataj.result;
                                                    row.save(function (error) {
                                                        logger.info("updateWithdrawalStatus success.");
                                                    })
                                                });
                            ...
                            db.models.gateway_erc20_withdrawal.findAsync({
                                ...
                                .each(function (item) {
                                    db.models.gateway_transaction.create({}, function (err, trans) {
                                        processWithdrawal(trans, item, withdrawalKeys)
                                    });
                                });
                        }
                    })
