...
var processWithdrawal = function (request, withdrawalKey) {
    database('localhost', 'main').then(function (db) {
            ...
            .then(function (row) {
                ...
                row.save(function (err) {
                    if (err) {
                        ...
                        Payment.erc20Payment(row.amount, withdrawalKey, config.erc20WithdrawalWallet, row.withdrawal_address, true, config.erc20.contractAddress, 18)
        ...
        database('localhost', 'main').then(function (db) {
            cron.schedule('*/1 * * * *', function () {
                db.models.gateway_erc20_withdrawal.findAsync({
                    ...
                    .each(function (item) {
                        processWithdrawal(item, withdrawalKey)
                    });
            })
        });
    });
